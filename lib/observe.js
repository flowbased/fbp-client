...
function signalToError(signal) {
  ...
  if (signature === 'network:data' && signal.payload.src && signal.payload.src.port === 'error') {
    // Use the error message of the actual payload
    errorMessage = signal.payload.data.message || signal.payload.data;
    errorStack = signal.payload.data.stack;
  }
